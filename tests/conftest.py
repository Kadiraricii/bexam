...
# CANLI DOGRULANAN bir cokme (hem yerelde HEM GERCEK GITHUB ACTIONS CI'DA,
# macOS VE Windows'ta AYRI AYRI): tum test suite'i TEK bir surecte
...
# alt yapisi) ic durumuyla catisip surecin TAMAMEN cokmesine yol
# acabiliyordu - macOS'ta "Fatal Python error: Aborted", Windows'ta
# "Windows fatal exception: code 0x80000003" (bkz. asagidaki tk_root
# fixture'indeki benzer, ONCEDEN bulunmus Tk+threading cokme kategorisi -
# bu AYNI ailenin yeni bir ornegi).
#
# ASIL/KOKTEN cozum burada DEGIL, .github/workflows/tests.yml'de:
# test_browser_launch.py artik AYRI bir pytest cagrisinda (dolayisiyla
# AYRI bir Python surecinde) calistiriliyor - iki kirilgan kategori artik
# HICBIR ZAMAN ayni surecte bulusmuyor, bu deterministik bir cozum.
#
# Asagidaki gc.freeze() ise IKINCI bir savunma hatti: gelistiricinin
# yerelde TUM test dosyalarini (test_browser_launch.py dahil) TEK
# `pytest tests/` cagrisiyla calistirdigi senaryoda hala bir katman
# koruma saglasin diye kaliyor. gc.disable() sorunu TAMAMEN cozuyor ama
# tum coplari toplamayi tamamen durdurup suiti ~20 kat yavaslatiyor
...
# ONEMLI: bu OLASILIKSAL bir azaltma, GARANTI DEGIL (CI'da hala
# gozlemlendi) - bu yuzden process-izolasyonu YUKARIDAKI ASIL cozum.
gc.collect()
gc.freeze()
